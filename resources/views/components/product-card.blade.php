   ...
   href="{{ route('product.view', $product->defaultUrl->slug) }}">
    <div class="overflow-hidden border rounded-lg aspect-w-1 aspect-h-1 position-relative">
        @if($product->brand->defaultUrl->slug == 'xt-labs')
            <img src="{{ asset('img/us.svg') }}" class="product-brand" style="width: 32px; height: 32px;" alt="">
        @else
            <img src="{{ asset('img/international.svg') }}" class="product-brand" style="width: 32px; height: 32px;" alt="">
        @endif
